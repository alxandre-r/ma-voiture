...
export default function FillFormModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (autoCloseOnSuccess) setTimeout(onClose, 1500);
    } catch (err: unknown) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Ajouter un plein" size="md">
      <form onSubmit={handleSubmit} className="space-y-2 max-w-md p-3 rounded-lg bg-white dark:bg-gray-800 text-gray-800 dark:text-white">
        {/* Vehicle Selection */}
