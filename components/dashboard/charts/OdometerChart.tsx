...
import { useContainerSize, useMonthTicks, useDateRange, useMobileDetection } from './ChartHelper';
...
export default function OdometerChart({ vehicles }: OdometerChartProps) {
  ...
  const getY = (value: number) => padding.top + (1 - (value - minValue) / range) * innerHeight;
      ...
      <div className="flex items-center justify-between px-2 mb-2">
        ...
        >
          ...
          <div className="flex justify-between w-full px-3 text-xs font-medium z-20">
            <span className={mode === 'normal' ? 'text-white' : 'text-gray-700 dark:text-gray-300'}>
              Total
            </span>
            <span
              className={mode === 'comparatif' ? 'text-white' : 'text-gray-700 dark:text-gray-300'}
            >
              Normalisé
            </span>
          </div>
            ...
            return (
              <g key={value}>
                ...
                <text
                  ...
                  y={y + 4}
                  textAnchor="end"
                  className="text-xs fill-gray-400 dark:fill-gray-400"
                >
                  {value} km
                </text>
              </g>
